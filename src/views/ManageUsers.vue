    ...
    methods: {
        async LoadAllUsers() {
            try {
                ...
                this.UsersData = this.input_value ? this.UsersData?.filter(user => user?.name.toLowerCase().includes(this.input_value.toLowerCase())) : data
            }
            catch (error) {
                console.log(error);
            }
        ...
        <div class="table-wrapper">
            ...
            <table class="products-table">
                ...
                <tbody class="table-body">
                    <tr v-for="user in this.UsersData" :key="user.id">

                        <td>
                            <img class="product-image" :src="user.avatar ? user.avatar : 'https://png.pngtree.com/png-vector/20220709/ourmid/pngtree-businessman-user-avatar-wearing-suit-with-red-tie-png-image_5809521.png'">
                        </td>
                        <td>{{ user.name  }}</td>
                        <td>{{ user.email }} so'm</td>
                        <td>{{ user.role === 'customer' ? 'Xaridor' : 'Admin' }} </td>
